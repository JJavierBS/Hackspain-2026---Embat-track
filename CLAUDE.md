...
Configuración privada: `.env.local` está ignorado y se crea sin sobrescribir con `configure-ia` o `Preparar-IA.ps1 -GuardarLocal`. La clave solo se entrega al proceso de preparación, no a builds ni servidores web. Verificar con `node --test scripts/local-settings.test.mjs` y `node scripts/check-private-config.mjs` antes de commit/push; el último no imprime la clave. `S7_EXPECT_HELMCODE=true` exige fuente Helmcode real en la prueba smoke.
