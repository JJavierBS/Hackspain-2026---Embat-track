...
Example:
    python scripts/scoring/health_scoring.py \
    --profile <profile> --alert-threshold <threshold>

    Weights are read from ``scripts/weights_calc/weights.json`` and anchors from
``backend/src/main/resources/scoring-config.yml``. Raw indicator values are
read from ``data/xray.duckdb`` table ``indicator_values_raw``.
...
DEFAULT_WEIGHTS_PATH = Path(__file__).resolve().parents[1] / "weights_calc" / "weights.json"
...
def validate_weighted_indicators(weights: dict[str, Any], indicators: dict[str, dict[str, Any]]) -> None:
    ...
    if missing:
        raise ValueError(
            "weights.json contains indicators without configured anchors: "
            + ", ".join(missing)
        )


def normalize_weight_names(weights: dict[str, Any]) -> dict[str, Any]:
    """Map legacy external AHP names to the backend's canonical indicator IDs."""
    normalized: dict[str, Any] = {}
    for indicator, weight in weights.items():
        canonical = INDICATOR_ALIASES.get(indicator, indicator)
        if canonical in normalized:
            raise ValueError(f"duplicate weight after indicator aliasing: {canonical}")
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help="JSON with weights; defaults to scripts/weights_calc/weights.json",
    )
    ...
    profile_name, weights = select_weights(weights_payload, args.profile)
...
def select_weights(payload: dict[str, Any], profile_name: str | None = None) -> tuple[str | None, dict[str, Any]]:
    """Read one external indicator-weight map without knowing profile names."""
    if "weights" in payload:
        if profile_name is not None:
            raise ValueError("--profile cannot be used with a single-profile weights JSON")
        return payload.get("name"), payload["weights"]

    profiles = payload.get("profiles", payload)
    if profile_name is None:
        if len(profiles) != 1:
            raise ValueError("--profile is required when the JSON contains multiple profiles")
        profile_name = next(iter(profiles))
    if profile_name not in profiles:
        raise KeyError(f"profile {profile_name!r} is not present in the weights JSON")

    selected = profiles[profile_name]
    weights = selected.get("weights", selected)
