# Health Scoring (herramienta de contraste offline)

> **Nada del producto lee estos scripts ni su salida** (fase 7, decisión H10). La puntuación del producto
> la calcula el pipeline del backend (S40–S70) y la proyección `ForecastCalculator` + `S85_Forecast`
> (ver `docs/FORECAST.md`). Estos scripts son una reimplementación independiente del término de nivel,
> en escala 0–1, para contrastar anchors y pesos.
...
- Anchors, categorías y pesos: `backend/src/main/resources/scoring-config.yml` (la única fuente de verdad)
- Valores reales de indicadores: `data/xray.duckdb`, tabla `indicator_values_raw`

No es necesario pasar estas rutas en la ejecución normal. Se pueden sobrescribir con `--config` y `--database`. El backend bloquea `data/xray.duckdb`: usa una copia.
...
El peso de un indicador es `scoring.profiles.<P>.weights[categoría] / 100 × scoring.indicators.<ID>.weight / suma de los pesos de su categoría`. MOMENTUM no tiene indicadores: sale del cálculo y el resto se renormaliza.
...
Los perfiles son las claves de `scoring.profiles`: `BANK`, `FUND` e `INSURER`.

El script renormaliza los pesos entre los indicadores disponibles.
...
```bash
cd scripts/scoring && python -m unittest test_health_scoring test_cusum_forecast
```
