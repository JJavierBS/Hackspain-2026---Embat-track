...
| M8 | An entity whose available categories carry less than `confidence.min-trusted-weight-share` of the profile weight gets confidence `INSUFFICIENT`. It is scored and shown, and it leaves the ranking, the bands and the filters. | Thin evidence produces a number that looks like the others and is not comparable to them. Hiding the entity would be worse: the reader must see that it exists. | `DATA_FINDINGS.md` "Coverage gate" |
